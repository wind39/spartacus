...
try:
    ...
    v_table_a = v_database.Query(
        "select dept_no, dept_name, 'A' as test from departments"
    )
    ...
    v_table_b = v_database.Query(
        "select dept_no, dept_name, 'B' as test from departments"
    )
