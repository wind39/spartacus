...
class TestSQLite(unittest.TestCase):
    ...
    def test_execute(self):
        v_database = Spartacus.Database.SQLite('employees.db')
        v_database.Open()
        v_database.Execute("insert into departments (dept_no, dept_name) values ('d000', 'Spartacus')")
        v_result = v_database.ExecuteScalar("select dept_name from departments where dept_no = 'd000'")
    ...
    def test_getfields(self):
        v_database = Spartacus.Database.SQLite('employees.db')
        v_result = v_database.GetFields("select 1 as id, 'Spartacus' as name, '1988-05-08 17:00:00' as 'birth_date [timestamp]', 9.8 as grade")
    ...
    def test_query_simple(self):
        v_database = Spartacus.Database.SQLite('employees.db')
        v_result = v_database.Query("select * from departments order by dept_no", p_simple=True)
        self.assertIsInstance(v_result, Spartacus.Database.DataTable)
        v_template = ['dept_no', 'dept_name']
        self.assertListEqual(v_result.Columns, v_template)
        self.assertEqual(len(v_result.Rows), 9)
    ...
    def test_query_types(self):
        v_database = Spartacus.Database.SQLite('employees.db')
        v_result = v_database.Query("select 1 as id, 'Spartacus' as name, '1988-05-08 17:00:00' as 'birth_date [timestamp]', 9.8 as grade")
        self.assertIsInstance(v_result, Spartacus.Database.DataTable)
        ...
        self.assertEqual(v_result.Rows[0]['birth_date'], datetime.datetime.strptime('1988-05-08 17:00:00', '%Y-%m-%d %H:%M:%S'))
    ...
    def test_query_alltypesstr(self):
        v_database = Spartacus.Database.SQLite('employees.db')
        v_result = v_database.Query("select 1 as id, 'Spartacus' as name, '1988-05-08 17:00:00' as 'birth_date [timestamp]', 9.8 as grade", p_alltypesstr=True)
        self.assertIsInstance(v_result, Spartacus.Database.DataTable)
        v_template = ['id', 'name', 'birth_date', 'grade']
        self.assertListEqual(v_result.Columns, v_template)
        self.assertEqual(len(v_result.Rows), 1)
    ...
    def test_queryblock_connection_not_open(self):
        ...
        with self.assertRaises(Spartacus.Database.Exception):
            v_result = v_database.QueryBlock("select * from departments order by dept_no", 4)
    ...
    def test_insertblock(self):
        ...
        v_result = v_database.Query("select * from departments where dept_no in ('d010', 'd011')")
    ...
    def test_insertblock_fields(self):
        ...
        v_result = v_database.Query("select * from employees where emp_no in (500000, 500001)")
        self.assertEqual(len(v_result.Rows), 2)
