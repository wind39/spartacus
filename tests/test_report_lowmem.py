...
try:
    ...
    v_totalRows = int(v_database.ExecuteScalar('select count(*) from employees'))
    if v_totalRows == 0:
        exit(0)
    ...
    v_boldFont = openpyxl.styles.Font(
        bold = True
    )
    ...
    v_headerFont = openpyxl.styles.Font(
        bold = True
    )

    v_headerFill = openpyxl.styles.PatternFill(
        'solid',
        fgColor = 'DBE5F1'
    )
    ...
    for v_line in Spartacus.Report.AddTable(p_workSheet = v_workSheet, p_headerDict = v_headerDict, p_startColumn = 1, p_startRow = 6, p_headerHeight = 40, p_database = v_database, p_query = v_query, p_mainTable = True):
        if v_line % 1000 == 0:
            v_progress += v_inc * 1000
            print('{0}: Rendering line {1} of {2} - {3}%'.format(v_reportName, v_line, v_totalRows, v_progress))
